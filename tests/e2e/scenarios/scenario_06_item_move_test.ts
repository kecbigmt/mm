...
describe("Scenario 6: Item movement (move)", () => {
  ...
  it("moves item to head using head: placement", async () => {
    ...
    const ls1Result = await runCommand(ctx.testHome, ["ls"], { mmCwd: cdToday.mmCwd! });
    ...
    // Get task C ID
    const today = await getCurrentDateFromCli(ctx.testHome, { mmCwd: cdToday.mmCwd! });
    ...
    const ls2Result = await runCommand(ctx.testHome, ["ls"], { mmCwd: cdToday.mmCwd! });
  ...
  it("moves item using after: relative positioning", async () => {
    ...
    // Create three tasks
    await runCommand(ctx.testHome, ["note", "タスクA"], { mmCwd: cdToday.mmCwd! });
    await runCommand(ctx.testHome, ["note", "タスクB"], { mmCwd: cdToday.mmCwd! });
    await runCommand(ctx.testHome, ["note", "タスクC"], { mmCwd: cdToday.mmCwd! });
    ...
    await runCommand(ctx.testHome, ["mv", taskCId, "head:today"], { mmCwd: cdToday.mmCwd! });
  ...
  it("moves item to different parent/section", async () => {
    ...
    const cdProject = await runCd(ctx.testHome, "project/1", { mmCwd: cdToday.mmCwd! });
    assertEquals(cdProject.success, true, `cd failed: ${cdProject.stderr}`);

    // Verify task A is in project/1
    const lsResult = await runCommand(ctx.testHome, ["ls"], { mmCwd: cdProject.mmCwd! });
    ...
    const cdBackToday = await runCd(ctx.testHome, "today", { mmCwd: cdProject.mmCwd! });
    assertEquals(cdBackToday.success, true, `cd today failed: ${cdBackToday.stderr}`);

    // Verify task A is no longer in today
    const lsTodayResult = await runCommand(ctx.testHome, ["ls"], { mmCwd: cdBackToday.mmCwd! });
  ...
  it("executes full flow: create → move head → move after → move to parent", async () => {
    ...
    const taskAResult = await runCommand(ctx.testHome, ["note", "タスクA"], { mmCwd: currentCwd });
    assertEquals(taskAResult.success, true, "Task A creation should succeed");

    const taskBResult = await runCommand(ctx.testHome, ["note", "タスクB"], { mmCwd: currentCwd });
    assertEquals(taskBResult.success, true, "Task B creation should succeed");

    const taskCResult = await runCommand(ctx.testHome, ["note", "タスクC"], { mmCwd: currentCwd });
    ...
    const today = await getCurrentDateFromCli(ctx.testHome, { mmCwd: currentCwd });
    const taskCId = await getItemIdByTitle(ctx.testHome, "test-workspace", today, "タスクC");
    assertExists(taskCId, "Task C ID should be found");
    ...
    const ls3Result = await runCommand(ctx.testHome, ["ls"], { mmCwd: currentCwd });
    ...
    const cdProject = await runCd(ctx.testHome, "project/1", { mmCwd: currentCwd });
    assertEquals(cdProject.success, true, "cd project/1 should succeed");
    currentCwd = cdProject.mmCwd!;

    // Step 13: Verify task A is in project/1
    const ls4Result = await runCommand(ctx.testHome, ["ls"], { mmCwd: currentCwd });
    ...
    const cdBackToday = await runCd(ctx.testHome, "today", { mmCwd: currentCwd });
    assertEquals(cdBackToday.success, true, "cd today should succeed");
    currentCwd = cdBackToday.mmCwd!;

    // Step 15: Verify task A is no longer in today
    const ls5Result = await runCommand(ctx.testHome, ["ls"], { mmCwd: currentCwd });
