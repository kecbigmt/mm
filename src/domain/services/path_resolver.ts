...
export const createPathResolver = (
  ...
  const resolveRelativeDate = (expr: string): Result<CalendarDay, PathResolverError> => {
    ...
    if (keywordOffset !== undefined) {
      ...
      return parseCalendarDay(dateStr).mapError((err) =>
        createValidationError(PATH_RESOLVER_ERROR_KIND, err.issues)
      );
    }
    ...
    return parseCalendarDay(expr).mapError((err) =>
      createValidationError(PATH_RESOLVER_ERROR_KIND, err.issues)
    );
  };
  ...
  const resolveToken = async (
    ...
    switch (token.kind) {
      ...
      case "dotdot": {
        // Parent - pop one section or move to parent head
        const parent = context.stack.parent();
        if (parent === null) {
          return Result.error(
            createValidationError(PATH_RESOLVER_ERROR_KIND, [
