    ...
    try {
      ...
      if (remoteExists) {
        ...
        if (!options?.force) {
          return Result.error({
            kind: "VersionControlError",
            message: `Remote '${name}' already exists with different URL: '${existingUrl}'. Expected: '${url}'. Use --force to overwrite.`,
          });
        }

        // Force overwrite
        return safeExecute("git", ["remote", "set-url", name, url], cwd, "git remote set-url failed");
      }
