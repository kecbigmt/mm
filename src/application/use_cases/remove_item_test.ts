...
import { removeItem } from "./remove_item.ts";
...
const createTestItem = (idSuffix: string) => {
...
Deno.test("removeItem returns structured partial-success DTOs", async () => {
  ...
  const result = await removeItem({
    itemIds: [item.data.id.toString(), missingId],
  ...
  assertEquals(result.value.succeeded[0].id, item.data.id.toString());
...
Deno.test("removeItem returns validation error for empty itemIds", async () => {
  const deps = createDeps();
