...
export type SyncInitDeps = Readonly<{
...
export type SyncInitApplicationError =
  | VersionControlError
  | RepositoryError
  | ValidationError<string>;

export type SyncInitResponse = Readonly<{
  branch: string;
  remote: string;
}>;
...
export const syncInit = async (
  input: SyncInitRequest,
  deps: SyncInitDeps,
): Promise<Result<SyncInitResponse, SyncInitApplicationError>> => {
  // 0. Validate Input
