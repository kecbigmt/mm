...
3. Execute `git pull --ff-only <remote> <branch>`.
4. If not fast-forward, Git's error message is displayed.
...
If `git pull --ff-only` fails, Git's error message is displayed.
