...
```
mm sync init <remote-url> [--branch <branch-name>]
```
...
2. If not already a Git repo:
```sh
git init
```

3. Set or update remote:

```sh
git remote remove origin   # only if origin exists
git remote add origin <url>
```

4. Write or update `workspace.json.git`:
...
1. Ensure `.gitignore` contains required entries.
2. Make an initial commit if necessary:
...
1. On failure, surface Git error.
...
A small abstraction layer (`GitClient`) is recommended.
...
* Warn if `origin` exists with different URL
