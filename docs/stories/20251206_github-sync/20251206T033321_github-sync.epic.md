...
5. Ensure `.gitignore` contains required entries.
6. Make an initial commit if necessary:
...
4. On failure, surface Git error.
...
* Fail if `origin` exists with different URL (use `-f` to overwrite)
