...
- [ ] **Given** `git.enabled=true` and `sync_mode="auto-commit"`, **When** I run `mm remove <path>`, **Then** a Git commit is created after the removal. *(Note: `mm remove` command not implemented yet)*
...
#### Remaining
- `mm remove` command not yet implemented, so auto-commit integration pending for that command
