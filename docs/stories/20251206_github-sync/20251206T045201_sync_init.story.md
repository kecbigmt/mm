...
- [x] **Given** a workspace that is not a git repo, **When** I run `mm sync init https://github.com/user/repo.git`, **Then** a git repository is initialized in the workspace root.
- [x] **Given** a new git repo, **When** initialized, **Then** the remote `origin` is set to the provided URL.
- [x] **Given** an existing git repo with the *same* remote URL, **When** I run `mm sync init`, **Then** it succeeds idempotently.
- [x] **Given** an existing git repo with a *different* remote URL, **When** I run `mm sync init`, **Then** it fails with an error to prevent accidental overwrite.
- [x] **Given** an existing git repo with a *different* remote URL, **When** I run `mm sync init --force`, **Then** the remote URL is updated to the new one.
- [x] **Given** `mm sync init` is run, **Then** `.gitignore` is created (or updated) to ignore `.state.json`, `.index/`, `.tmp/`, `.DS_Store`, and `*.swp`.
- [x] **Given** `mm sync init` is run, **Then** an initial commit is created with message "mm: initialize workspace git repository".

#### 2. Configuration Persistence
- [x] **Given** `mm sync init` completes, **When** I inspect `workspace.json`, **Then** it has a `git` object with `enabled: true`, `remote: <url>`, `branch: "main"`, and `sync_mode: "auto-commit"`.
- [x] **Given** valid input, **When** I run `mm sync init <url> --branch develop`, **Then** `workspace.json` records `branch: "develop"`.

#### 3. Error Cases
- [x] **Given** an invalid Git URL format, **When** I run `mm sync init <invalid-url>`, **Then** it returns an error and does not modify the workspace.
- [x] **Given** an invalid branch name, **When** I run `mm sync init`, **Then** it returns a specific validation error "Invalid branch name: <branch>".
- [x] **Given** Git is not installed or a system git error occurs (e.g. permission check), **When** command runs, **Then** it fails gracefully with the original Git error message (not masked as a validation error).
