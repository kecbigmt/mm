...
Status: Draft Target version: mm v0.2.0
...
- **Recent Item Caching**: A mechanism to store aliases and context tags of recently
  accessed/modified items.
...
- **Workspace-based Caching**: Cache files stored within the declared workspace
  (`.index/completion_aliases.txt` and `.index/completion_context_tags.txt`).
- **Simple Text Format**: Plain text files, one value per line. File order represents recency
  (newest at end).
...
- Context-aware filtering (e.g., filtering `close` candidates to only 'open' items) beyond basic ID
  availability.
...
Users frequently need to reference items (by alias or context tag) across multiple commands. Typing
or remembering exact aliases and tags is cumbersome. Shell completion should suggest recently used
items to accelerate workflows.

Design goals:

1. **Recall**: Allow completion of recently accessed aliases and context tags, including those from
   closed items.
...
4. **Simplicity**: Cache-only approach; natural usage (`mm ls`, `mm note`, etc.) builds the cache
   over time.
...
1. **Cache Lookup**:
   - Resolve workspace root from `MM_HOME/config.json` (same as CLI)
   - Read `currentWorkspace` from config, default to `"home"` if not found
   - Locate cache files at `${MM_HOME}/workspaces/${currentWorkspace}/.index/`
   - Read file content directly (no parsing needed - plain text)
   - _Goal_: Low latency (< 20ms) for reading config and cache files
   - _Note_: Works from any directory - not dependent on CWD
2. **No Cache Behavior**:
...
- **Stateful Completion**: Completion candidates evolve based on usage. Natural command usage
  (`mm ls`, `mm note`, etc.) populates the cache.
- **Cache-Only**: If cache is empty (e.g., fresh install), no completion candidates are provided
  until commands are run.
- **Aliases and Tags Only**: Completion suggests aliases and context tags. IDs are not suggested
  since they are long UUIDs not meant for manual typing.
...
- **From results only**: Items created, displayed, or modified by the command (extract their aliases
  and tags).
...
- **JSONL Format**: Rejected in favor of plain text for simplicity and easier shell script
  integration.
...
- **Reuse `.index` directly**: Rejected due to latency (>50ms parsing thousands of files) and lack
  of recency history.
- **Fallback to full enumeration**: Rejected due to scalability concerns. Cache-only approach is
  simpler and safer; natural usage builds the cache.

---

## 6. Error Handling

- **Cache Write Failures**: Commands show warning message but continue execution (e.g., "Warning:
  Failed to update completion cache: PermissionDenied...").
- **Workspace Resolution Failure**: If MM_HOME/config.json is missing or unreadable, completion
  returns no candidates silently.
...
## 8. Shell Snippet (Actual Implementation)
...
2. **Cache Population**:
   ...
   - Check cache files exist in current workspace:
     `${MM_HOME}/workspaces/${currentWorkspace}/.index/completion_aliases.txt` and
     `.index/completion_context_tags.txt`
...
3. **Completion After Cache Population**:
   ...
   - Note: Completion works from any directory, not just inside workspace (uses
     MM_HOME/config.json).
